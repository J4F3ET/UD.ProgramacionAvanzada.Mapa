	...
	Microsoft.Maps.loadModule("Microsoft.Maps.SpatialMath", async function () {
		...
		try {
			...
			for (let i = 0; i < coordinates.length; i++) {
				let latitud = Microsoft.Maps.Location.normalizeLongitude(
					coordinates[i][1]
				);
				let longitud = Microsoft.Maps.Location.normalizeLongitude(
					coordinates[i][0]
				);
...
/*
					...
					if (Microsoft.Maps.SpatialMath.containsLocation(loc, poly)) {
						var pin = new Microsoft.Maps.Pushpin(loc);
						map.entities.push(pin);
						console.log('SIN ERROR ',i);
					}
*/
